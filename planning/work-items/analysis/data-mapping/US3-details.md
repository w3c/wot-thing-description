...
**Recommended use:** Use QUDT at property level for quantity/unit annotations when the transform is a genuine unit conversion. QUDT explains _what a value means_; `map` explains _how a TD runtime converts it_.
...
**Recommended use:** Use FnO as an optional enrichment layer for describing reusable transformation functions. FnO explains _which function is applied and how it is composed_; `map` remains needed for _where in the TD form that function is invoked_.
      ...
      "forms": [
        {
          ...
          "op": ["readproperty"],
          "map:valueMapping": {
            "map:fromWire": [{ "map:proc": "mul", "map:value": 0.1 }]
          }
        }
      ]
    }
...
- QUDT annotates the property-level unit (`unit:DEG_C`) to capture _semantics_.
- `map:valueMapping` with `map:fromWire` captures the _runtime execution_ step.
...
| ----------- | ------- | ------------------------ | ------------------------------------------------------------------ |
| _(none)_    | —       | —                        | No `bacv:*` term corresponds to `mul`, `add`, `round`, or `clamp`. |
...
| ------------- | ------- | ------------------------ | ------------------------------------------------------------------- |
| _(none)_      | —       | —                        | No `profv:*` term corresponds to `mul`, `add`, `round`, or `clamp`. |
