...
| oneOf           | Array of two or more security security definitions any one of which, when satisfied, will allow access. Only one may be chosen for use. | optional   | Array of Strings | only allowed when `scheme` is equal to `combo`                                                                                                                 |
| allOf           | Array of two or more security security definitions all of which must be satisfied for access.                                           | optional   | Array of Strings | only allowed when `scheme` is equal to `combo`                                                                                                                 |
