...
const show_test_specs = undefined != config.show_test_specs ? config.show_test_specs : false;

// include interop test results in report
const show_interop_results = undefined != config.show_interop_results ? config.show_interop_results : false;

//==== Directories

// source directory (location of specification)
const src_dir = undefined != config.src_dir ? path.join(__dirname, config.src_dir) : __dirname;

// testing directory
const testing_dir_rel = undefined != config.testing_dir_rel ? config.testing_dir_rel : "testing";
const testing_dir =
  undefined != config.testing_dir ? path.join(__dirname, config.testing_dir) : path.join(src_dir, testing_dir_rel);

// target directory for report output
const report_dir = undefined != config.report_dir ? path.join(__dirname, config.report_dir) : testing_dir;

// location of other inputs
const inputs_dir_rel = undefined != config.inputs_dir_rel ? config.inputs_dir_rel : "inputs";
const inputs_dir =
  undefined != config.inputs_dir ? path.join(__dirname, config.inputs_dir) : path.join(testing_dir, inputs_dir_rel);

// location of HTML templates
const templates_dir_rel = undefined != config.templates_dir_rel ? config.templates_dir_rel : "templates";
const templates_dir =
  undefined != config.templates_dir
    ? path.join(__dirname, config.templates_dir)
    : path.join(inputs_dir, templates_dir_rel);

// implementation descriptions
const descs_dir_rel = undefined != config.descs_dir_rel ? config.descs_dir_rel : "implementations";
const descs_dir =
  undefined != config.descs_dir ? path.join(__dirname, config.descs_dir) : path.join(inputs_dir, descs_dir_rel);

// test results for each assertion and impl
const results_dir_rel = undefined != config.results_dir_rel ? config.results_dir_rel : "results";
const results_dir =
  undefined != config.results_dir ? path.join(__dirname, config.results_dir) : path.join(inputs_dir, results_dir_rel);

// interop test results directory
const interop_dir_rel = undefined != config.interop_dir_rel ? config.interop_dir_rel : "interop";
const interop_dir =
  undefined != config.interop_dir ? path.join(__dirname, config.interop_dir) : path.join(inputs_dir, interop_dir_rel);

//==== Inputs

// source specification (rendered)
const src_htmlfile_rel = undefined != config.src_htmlfile_rel ? config.src_htmlfile_rel : "index.html";
const src_htmlfile =
  undefined != config.src_htmlfile ? path.join(__dirname, config.src_htmlfile) : path.join(src_dir, src_htmlfile_rel);

// report template
const rt_htmlfile_rel = undefined != config.rt_htmlfile_rel ? config.rt_htmlfile_rel : "report.html";
const rt_htmlfile =
  undefined != config.rt_htmlfile
    ? path.join(__dirname, config.rt_htmlfile)
    : path.join(templates_dir, rt_htmlfile_rel);

// testspec template
const tt_htmlfile_rel = undefined != config.tt_htmlfile_rel ? config.tt_htmlfile_rel : "testspec.html";
const tt_htmlfile =
  undefined != config.tt_htmlfile
    ? path.join(__dirname, config.tt_htmlfile)
    : path.join(templates_dir, tt_htmlfile_rel);

// interop template
const it_htmlfile_rel = undefined != config.it_htmlfile_rel ? config.it_htmlfile_rel : "interop.html";
const it_htmlfile =
  undefined != config.it_htmlfile
    ? path.join(__dirname, config.it_htmlfile)
    : path.join(templates_dir, it_htmlfile_rel);

// test specifications for assertions
const ts_htmlfile_rel = undefined != config.ts_htmlfile_rel ? config.ts_htmlfile_rel : "testspec.html";
const ts_htmlfile =
  undefined != config.ts_htmlfile ? path.join(__dirname, config.ts_htmlfile) : path.join(inputs_dir, ts_htmlfile_rel);

// extra non-spec assertions (e.g. child assertions)
const ea_htmlfile_rel = undefined != config.ea_htmlfile_rel ? config.ea_htmlfile_rel : "extra-asserts.html";
const ea_htmlfile =
  undefined != config.ea_htmlfile ? path.join(__dirname, config.ea_htmlfile) : path.join(inputs_dir, ea_htmlfile_rel);

// assertion dependencies
const depends_csvfile_rel = undefined != config.depends_csvfile_rel ? config.depends_csvfile_rel : "depends.csv";
const depends_csvfile =
  undefined != config.depends_csvfile
    ? path.join(__dirname, config.depends_csvfile)
    : path.join(inputs_dir, depends_csvfile_rel);

// assertion categories
const categories_csvfile_rel =
  undefined != config.categories_csvfile_rel ? config.categories_csvfile_rel : "categories.csv";
const categories_csvfile =
  undefined != config.categories_csvfile
    ? path.join(__dirname, config.categories_csvfile)
    : path.join(inputs_dir, categories_csvfile_rel);

// at-risk assertions (input)
const atrisk_csvfile_rel = undefined != config.atrisk_csvfile_rel ? config.atrisk_csvfile_rel : "atrisk.csv";
const atrisk_csvfile =
  undefined != config.atrisk_csvfile
    ? path.join(__dirname, config.atrisk_csvfile)
    : path.join(inputs_dir, atrisk_csvfile_rel);

// structured implementation data
const impls_csvfile_rel = undefined != config.impls_csvfile_rel ? config.impls_csvfile_rel : "impl.csv";
const impls_csvfile =
  undefined != config.impls_csvfile
    ? path.join(__dirname, config.impls_csvfile)
    : path.join(inputs_dir, impls_csvfile_rel);

// assertions to be tested manually
const manual_csvfile_rel = undefined != config.manual_csvfile_rel ? config.manual_csvfile_rel : "manual.csv";
const manual_csvfile =
  undefined != config.manual_csvfile
    ? path.join(__dirname, config.manual_csvfile)
    : path.join(testing_dir, manual_csvfile_rel);

// assertions to be suppressed from IR
const suppressed_csvfile_rel =
  undefined != config.suppressed_csvfile_rel ? config.suppressed_csvfile_rel : "suppressed.csv";
const suppressed_csvfile =
  undefined != config.suppressed_csvfile
    ? path.join(__dirname, config.suppressed_csvfile)
    : path.join(testing_dir, suppressed_csvfile_rel);

//==== Outputs

// report filename
const report_htmlfile_rel = undefined != config.report_htmlfile_rel ? config.report_htmlfile_rel : "report.html";
const report_htmlfile =
  undefined != config.report_htmlfile
    ? path.join(__dirname, config.report_htmlfile)
    : path.join(report_dir, report_htmlfile_rel);

// all assertions
const report_csvfile_rel = undefined != config.report_csvfile_rel ? config.report_csvfile_rel : "template.csv";
const report_csvfile =
  undefined != config.report_csvfile
    ? path.join(__dirname, config.report_csvfile)
    : path.join(report_dir, report_csvfile_rel);

// at-risk assertion styling
const atrisk_cssfile_rel = undefined != config.atrisk_cssfile_rel ? config.atrisk_cssfile_rel : "atrisk.css";
const atrisk_cssfile =
  undefined != config.atrisk_cssfile
    ? path.join(__dirname, config.atrisk_cssfile)
    : path.join(testing_dir, atrisk_cssfile_rel);

// at-risk assertions (output)
const atrisk_out_csvfile_rel =
  undefined != config.atrisk_out_csvfile_rel ? config.atrisk_out_csvfile_rel : "atrisk.csv";
const atrisk_out_csvfile =
  undefined != config.atrisk_out_csvfile
    ? path.join(__dirname, config.atrisk_out_csvfile)
    : path.join(testing_dir, atrisk_out_csvfile_rel);
...
// file...
const report_base = undefined != config.report_base ? config.report_base : "";
// path.join(report_dir, "report.html");

// Base URL for specification.
// const src_base = "https://www.w3.org/TR/wot-thing-description";
const src_base = undefined != config.src_base ? config.src_base : "https://w3c.github.io/wot-thing-description";
...
const repeat_assertion_in_appendix =
  undefined != config.repeat_assertion_in_appendix ? config.repeat_assertion_in_appendix : false;

//==== Other

// Verbosity level
const verbosity = undefined != config.verbosity ? config.verbosity : 0;
...
let lastModifiedString =
  lastModified.getDate() + " " + monthNames[lastModified.getMonth()] + " " + lastModified.getFullYear();
...
src_dom(".rfc2119-assertion, .rfc2119-table-assertion, .rfc2119-default-assertion").each(function (i, elem) {
  ...
  if ("rfc2119-table-assertion" === cl) {
    if (undefined === id) {
      ...
      let assertion_data = src_dom(this)
        .children("td")
        .map(function (i, el) {
      ...
      let assertion =
        '<span class="rfc2119-table-assertion">' +
  ...
  if ("rfc2119-default-assertion" === cl) {
    if (undefined === id) {
      ...
      let assertion_data = src_dom(this)
        ...
        .get();
      let assertion =
        '<span class="rfc2119-default-assertion">' +
  ...
  csvtojson()
    ...
    .then((data) => {
      for (let i = 0; i < data.length; i++) {
        ...
        if (undefined !== producer && undefined !== consumer && undefined !== security) {
          interop.add({
            producer: producer,
            consumer: consumer,
            security: security,
          ...
          let new_entry = "fail" === teststatus ? "Fail" : "nosec" === security ? "Pass" : "Secure";
          if (undefined === entry) {
            entry = new_entry;
...
function merge_interops(done_callback) {
  if (show_interop_results) {
    ...
    interop_producers.forEach(function (producer) {
      ...
      report_tr.append('\n<tr><th id="' + producer + '">' + impl_org + "<br/>" + impl_name + "</th></tr>");
      interop_consumers.forEach(function (consumer) {
...
function format_assertions(done_callback) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (undefined !== suppressed.get(a)) {
      ...
      // Make table row
      let d = depends.get(a);
      report_dom("table#" + tid + ">tbody:last-child").append('\n<tr id="' + a + '" class="' + ac + '"></tr>');
                                    ...
                                    if (error) {
                                      ...
                                      output_atrisk(function (error) {
                                        if (error) {
                                          ...
                                          if (info_v)
                                            console.log("Flagged atrisk assertions output to " + atrisk_out_csvfile);
                                        }
                                      });
                                    }
                                  });
